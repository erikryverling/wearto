...
private const val BACKUP_FILE_NAME = "wearto-backup.csv"
...
class MainActivity : ComponentActivity() {
    ...
    private val createFileLauncher = buildLauncher(R.string.export_error_message) { uri ->
        lifecycleScope.launch {
            viewModel.getItemsAsCsv().collect { csv ->
                saveTextToFile(
                    uri = uri,
                    text = csv,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            WearToTheme {
                ...
                if (state.value is MainViewModel.UiState.Message) {
                    LaunchedEffect(state) {
                        scope.launch {
                            val message = (state.value as MainViewModel.UiState.Message).message
    ...
    private fun buildLauncher(@StringRes errorMessage: Int, onSuccess: (Uri) -> Unit) =
        registerForActivityResult(StartActivityForResult()) { result: ActivityResult ->
            if (result.resultCode == RESULT_OK) {
                val data = result.data?.data?.let { uri ->
                    onSuccess(uri)
                }
                if (data == null) {
