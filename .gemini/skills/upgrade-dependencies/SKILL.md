...
4. Install the app in the mobile:app module
5. Verify that it starts
6. Start the Wear_OS_Small_Round_API_36 Android emulator
7. Install the app in the wear:app module
...
### RC Versions Are Not Stable
**Problem**: When aiming for stable versions, web searches for "latest" might return Release Candidate (RC) versions, which are not considered stable.
- **Example**: Searching for "latest stable Kotlin" returned `2.3.20-RC`, while the actual latest stable was `2.3.10`.
- **Solution**: Always confirm if a version is truly stable (no RC, Beta, Alpha suffixes). If only RCs are newer, stick to the current stable version and document the reason.

### Hilt and AGP Incompatibility
**Problem**: Upgrading Hilt can introduce incompatibility with the Android Gradle Plugin (AGP), especially if newer AGP versions are not yet publicly available on Maven Central.
- **Example**: Hilt `2.59.1` requires AGP `9.0.0+`, but AGP `9.x` was not available, leading to compilation failures with AGP `8.13.1`.
- **Solution**: Always check Hilt's compatibility matrix with AGP. If the required AGP version is unavailable, revert Hilt to a compatible version and document the constraint.

### `kotlin-dsl-plugin` Version Mismatch
**Problem**: Explicitly setting the `kotlin-dsl-plugin` version might conflict with the version Gradle expects for its current distribution, leading to warnings or build failures.
- **Example**: Manually upgrading `kotlin-dsl-plugin` to `6.5.2` caused a warning that Gradle `9.3.1` expects `6.4.2`.
- **Solution**: It's best to let Gradle manage the `kotlin-dsl-plugin` version by removing explicit version constraints from `libs.versions.toml` or `build.gradle.kts`. If a specific version is required, ensure it's compatible with the Gradle version in use.
